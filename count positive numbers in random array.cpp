...
void fillAndPrintRandomArray(int arr1[100], int& num)
{
	...
	for (int a = 0; a < num; a++)
	{
		arr1[a] = randomnumbers(-100, 100);
		cout << arr1[a] << " ";
	}
}
int positivenumberscount(int arr1[100], int num)
{
	int count = 0;
	for (int b = 0; b < num; b++)
	{
...
void printpositivenumberscount(int x)
{
	cout << "positive number count is " << x << endl;
}
int main()
{
	...
	printpositivenumberscount(positivenumberscount(j, r));
